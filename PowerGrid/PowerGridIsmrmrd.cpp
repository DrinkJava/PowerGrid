...
/*****************************************************************************
    ...
    Description [This reconstruction supports 2D and 3D reconstructions with
    				time segmentation for field correction. This support is experimental.]
...
{
  std::string rawDataFilePath, outputImageFilePath, senseMapFilePath, fieldMapFilePath, precisionString, TimeSegmentationInterp, rawDataNavFilePath;
  ...
  try
	...
	typedef double PGPrecision;
	std::cout << "Did not recognize precision option. Defaulting to double precision." << std::endl;
	}
      */
	...
	type = 1;
	std::cout <<
	  "Did not recognize temporal interpolator selection. Acceptable values are hanning or minmax." << std::endl;
      }
    }
  ...
  for( u_int ii = 0; ii < numAcq; ii++ ) {											 
    ...
    TimeSegmentation<double, Gnufft<double> > A(Gg, vectorise(FM), vectorise(tvec), nro, Nx * Ny * Nz, L, type, NShots);
    SENSE<double, TimeSegmentation<double, Gnufft<double> > > Sg(A, sen, nro, Nx * Ny * Nz, nc);			
    QuadPenalty<double> R(Nx, Ny, Nz, beta);										 
    ImageTemp = reconSolve < double, SENSE<double, TimeSegmentation<double, Gnufft<double> > >, QuadPenalty <double> >	
  ...
  for (uword NRep = 0; NRep < NRepMax; NRep++) {												
	...
	Gnufft<double> Gg(kx.n_rows, 2.0, Nx, Ny, Nz, kx, ky, kz, vectorise(ix), vectorise(iy), vectorise(iz));
	TimeSegmentation<double, Gnufft<double> > TSeg(Gg, vectorise(FM), vectorise(tvec), kx.n_rows, Nx * Ny * Nz, L,type,NShots);
	//				Gdft<double> A(kx.n_rows, Nx * Ny * Nz,kx,ky,kz,vectorise(ix),vectorise(iy),vectorise(iz),vectorise(FM),vectorise(tvec));
	SENSE<double, TimeSegmentation<double, Gnufft <double> > > Sg(TSeg, sen, kx.n_rows, Nx * Ny * Nz, nc);	     	
	QuadPenalty<double> R(Nx, Ny, Nz, beta);
	ImageTemp = reconSolve < double, SENSE<double, TimeSegmentation<double, Gnufft<double> > > , QuadPenalty <double > >(data, Sg, R, kx, ky, kz, Nx, Ny, Nz, tvec, NIter);
	writeISMRMRDImageData<double>(d, ImageTemp, Nx, Ny, Nz);
      }																	
    }																	 
  }																	  
